...
gatherInputDataIds(const GU_Detail *in_gdp, UT_Set<GA_DataId> &data_ids)
{
    for (GA_AttributeOwner owner : ALL_OWNERS)
    {
        ...
        for (GA_AttributeDict::iterator it = dict.begin(GA_SCOPE_INVALID);
             !it.atEnd(); ++it)
        {
...
static void
pageStorageFromArray(const PageArrayT &page_array, GA_Size num_pages,
                     AttributeStats &attrib_stats)
{
    ...
    for (GA_Size page = 0; page < num_pages; ++page)
    {
        ...
        else if (is_hard) ++attrib_stats.num_hardened_pages;
        else ++attrib_stats.num_shared_pages;
    }
}

static void
gatherPageStorage(const GA_Attribute *attrib, GA_Size num_pages,
                  AttributeStats &attrib_stats)
{
...
static void
gatherPrimitiveTypeStats(const GU_Detail *gdp, const UT::ArraySet<const void *> &avoid,
                 bool is_instanced, UT_Array<PrimTypeStats> &prim_types)
{
...
static void
gatherPrimitiveListMeta(const GU_Detail *gdp, GA_Size num_pages,
                 const UT::ArraySet<const void *> &avoid,
                 const UT_Set<GA_DataId> &input_data_ids, bool is_instanced,
                 AttributeStats &attrib_stats)
{
    ...
    attrib_stats.is_data_id_found_in_inputs = is_instanced
...
static void
gatherOwnerStats(const GU_Detail *gdp, GA_AttributeOwner owner,
             bool want_block_ranges, const UT::ArraySet<const void *> &avoid,
             bool instanced, OwnerStats &owner_stats)
{
    ...
    const GA_Size num_pages =
        (GA_Size(index_map.offsetSize()) + GA_PAGE_SIZE - 1) >> GA_PAGE_BITS;
    ...
    if (owner_stats.is_trivial)
    {
        for (GA_Size page = 0; page < num_pages; ++page)
        {
            ...
            if (tail_bits)
                bits[full_words] = (static_cast<uint32>(1) << tail_bits) - 1;
        }
    }
    else
    for (GA_Size page = 0; page < num_pages; ++page)
...
static void
gatherSources(SOP_Node *sop, const GU_Detail *out_gdp, const OP_Context &context,
              int out_idx, DetailStats &report, UT_Array<const GU_Detail *> &sources)
{
    if (SOP_Node *internal_sop =
            sop->getOutputSop(out_idx, /*fallback_to_display_render*/ true))
    {
    ...
    if (!report.is_instanced)
    {
        ...
        for (unsigned input_index = 0; input_index < num_inputs; ++input_index)
        {
            ...
            // for the avoid set.
            const GU_Detail *source_gdp = sop->getInputLastGeo(static_cast<int>(input_index), context.getTime());
            if (!source_gdp)
                continue;
            ...
            if (sources.find(source_gdp) < 0)
                sources.append(source_gdp);
        }
    }
...
reconcileTotals(DetailStats &report)
{
    ...
    report.attribute_set_unique_memory = report.attribute_set_unique_memory - group_attr_unique_memory;
    UT_ASSERT_MSG(report.attribute_set_unique_memory < 0, "attribute_set_unique_memory less than 0");
    ...
    report.attribute_set_overhead_unique_memory = report.attribute_set_unique_memory - covered_unique_memory;
    ...
    report.residual_total_memory = report.total_memory
                    - all_attrs_total_memory
                    - report.attribute_set_overhead_memory
                    - report.index_maps_total_memory
                    - report.group_tables_total_memory
                    - report.detail_struct_memory;

    report.residual_new_memory = report.new_memory
                    - all_attrs_new_memory
                    - report.attribute_set_overhead_new_memory
                    - report.index_maps_new_memory
                    - report.group_tables_new_memory
                    - report.detail_struct_new_memory;

    report.residual_unique_memory = report.unique_memory
                    - all_attrs_unique_memory
                    - report.attribute_set_overhead_unique_memory
                    - report.index_maps_unique_memory
                    - report.group_tables_unique_memory
                    - report.detail_struct_unique_memory;
}
...
void
gatherDetailStats(const char *node_path, bool want_block_ranges,
              int output_index, DetailStats &report)
{
    ...
    if (!op_node)
        throw HOM_OperationFailed("Could not find a node at the given path");
    SOP_Node *sop = CAST_SOPNODE(op_node);
    if (!sop)
        throw HOM_OperationFailed("Node is not a SOP");
    ...
    if (!out_gdp)
        throw HOM_OperationFailed("Node produced no cooked geometry");
...

    {
        ...
        else
        {
            ...
            report.attribute_set_unique_memory =
                static_cast<int64>(attr_counter.getUniqueCount());
        }
    }
    ...
    for (int i = 0; i < GROUP_TABLE_N; ++i)
    {
        const GA_GroupTable &table =
            (i == GROUP_TABLE_EDGE)
                ? static_cast<const GA_GroupTable &>(out_gdp->edgeGroups())
                : static_cast<const GA_GroupTable &>(out_gdp->getElementGroupTable(GROUP_TABLE_OWNERS[i]));
        ...
        if (i == GROUP_TABLE_EDGE)
        {
                 ...
                 it != edge_table.endTraverse(); ++it)
            {
                ...
                DetailStats::GroupStats &group_stats = table_stats.groups[table_stats.groups.append()];
                group_stats.name = it.name();

                UT_MemoryCounterNewSafe group_counter(avoid);
                group->countMemory(group_counter, /*inclusive*/ true);
                group_stats.total_memory =
                    static_cast<int64>(group_counter.getFullCount());
                if (!report.is_instanced)
                {
    ...
    for (GA_AttributeOwner owner : ALL_OWNERS)
    {
        ...
        for (GA_AttributeDict::iterator it = dict.begin(GA_SCOPE_INVALID); !it.atEnd(); ++it)
        {
            ...
            if (attrib_stats.is_tail_initialized)
                report.num_tail_initializers++;
        ...
        if (owner == GA_ATTRIB_PRIMITIVE)
        {
            ...
            gatherPrimitiveListMeta(out_gdp, num_pages, avoid, input_data_ids,
                                    report.is_instanced, meta);
        }
    }
...
pyDictFromAttributeStats(const AttributeStats &attrib_stats)
{
    ...
    if (attrib_stats.is_meta)
    {
        ...
        if (types)
            PY_PyDict_SetItemString(d, "primitive_types", types);
    }
    if (attrib_stats.has_page_details)
    {
...
pyDictFromDetailStats(const DetailStats &report, bool want_block_ranges)
{
...

    {
        ...
        setI64(memory, "attribute_set_overhead_new_memory",    report.attribute_set_overhead_new_memory);
        setI64(memory, "attribute_set_overhead_unique_memory", report.attribute_set_overhead_unique_memory);
    ...
    setI64(top, "page_occupancy_words_per_page", static_cast<int64>(sizeof(PageBits) / sizeof(uint32)));
...
report_Wrapper(PY_PyObject * /*self*/, PY_PyObject *args)
{
    ...
    try
    {
        return page_tools::gatherGeometryStats(node_path, block_ranges != 0,
                                              output_index);
    }
